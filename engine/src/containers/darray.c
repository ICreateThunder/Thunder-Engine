...
/* [ CONSIDERATIONS ]
 *
 * - If field is valid (in range of ENUM)
 * - If points to a valid array (Value in Array Header)
 * 
 */
...
void* _darray_pop_at(void* array, u64 index, void* dest) {
  ...
  if (index >= length) {
    KERROR("Index oudside the bounds of this array! Length: %i, index: %i", length, index);
    return array;
  }
