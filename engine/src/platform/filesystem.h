...
// TODO: Refactor for platform-specific & ensure buffer overflow mitigation (reading and writing buffers)
...
/**
 * Attempt to open the file located at path
...
/**
 * Reads up to a newline or EOF character. Allocates *line_buf, which must be freed by the caller
 ...
 * 
 * *NOTE: Assumed max length of line 32000 characters when allowing function to allocate memory*
 */
KAPI b8 filesystem_read_line(file_handle* handle, char** line_buf);

/**
 * Writes text to the provided file, appending a '\n' afterward
 * @param handle A handle to the file_handle structure
